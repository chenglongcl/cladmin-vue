<template>
  <el-dialog title="上传文件" :close-on-click-modal="false" :before-close="handleClose" :visible.sync="uploaderVisible">
    <uploader ref="uploader" :key="uploaderKey" :autoStart="false" :options="options" :fileStatusText="fileStatusText" class="uploader-warpper" @file-progress="handleFileProgress" @file-added="handleFileAdded" @file-error="handleFileError" @file-success="handleSuccess">
      <uploader-unsupport></uploader-unsupport>
      <uploader-drop class="drop-warpper" v-if="fileList.length===0">
        <div class="panel">
          <p class="tip">点击选择素材或拖拽到本区域</p>
          <div>
            <uploader-btn :attrs="inputAttrs">点击选择素材</uploader-btn>
          </div>
        </div>
      </uploader-drop>
      <template v-else>
        <div class="upload-header btn-group">
          <div class="btn-warpper">
            <uploader-btn class="custom-btn" :attrs="inputAttrs">
              <i class="el-icon-plus"></i> <span>继续添加</span>
            </uploader-btn>
          </div>
        ...
        <div class="upload-footer btn-group">
          <span style="float: left;line-height:2.3">{{uploader.fileList.length}}个文件，{{stat.sizeUploaded | formatFileSize}} / {{uploader.getFormatSize()}}，上传成功{{stat.numberOfSuccessFiles}}个，失败{{stat.numberOfFailFiles}}个。</span>
    ...
    return {
      ...
      options: {
        target: (file, chunk, testMode) => {
          let url = `${file.requestUrl}?uploadId=${file.uploadId}`;
          if (testMode) {
            return url;
          } else {
        ...
        checkChunkUploadedByResponse: (chunk, message) => {
          ...
          let part = chunk.file.multipartUploadParts.find((item) => {
            return item.number == chunk.getParams().chunkNumber;
          });
          return (
            //本地和服务端都有分片记录，则认为已上传
            uploadedChunks.indexOf(chunk.getParams().chunkNumber) >= 0 && part
          );
        },
  ...
  methods: {
    ...
    async handleFileAdded(file) {
      ...
      try {
        ...
        file.fileUrl = initResult.fileUrl;
      } catch (error) {
        console.log(error);
        file.ignored = true;
      ...
      let ele = document.querySelector(".cd-scrollbar .el-scrollbar__wrap");
    ...
    async handleSuccess(rootFile, file, message, chunk) {
      this.myUploader.completeMultipartUpload(
        file.key,
        file.uploadId,
        file.multipartUploadParts
      );
    ...
    handleConfirm() {
      let unCompleteFiles = this.fileList.filter((file) => {
    ...
    // 弹窗关闭时
    handleClose(done) {
      let { fileList } = this.uploader;
      let unCompleteFiles = fileList.filter((file) => {
    ...
    handleMediaRes(type) {
      return {
        ...
        video: async function (file) {
          return new Promise((resolve) => {
            var videoUrl = URL.createObjectURL(file.file);
